---
url: https://articles.zsxq.com/id_68lgok59s1kn.html
title: 巨牛逼的产品做法 - 原型、skills、AI native
author: articles.zsxq.com
aliases: 
 - 
date: 2026-05-13 16:04:23
tags:

banner: "https://images.zsxq.com/FgeCV35RBepPB0Ub9Fq4icHAAyqA?e=2127196800&token=q6iZ0sQtf9U7s1qz0r4yMawNq3-u2w6lbnai6y2J:AoJZ9VINr-qkJk0CIgR84nxb2r8="
banner_icon: 🔖
---
# 巨牛逼的产品做法 - 原型、skills、AI native

[来自： 雷哥 AI 解决方案](https://wx.zsxq.com/group/28882285418421)

![](https://images.zsxq.com/FgeCV35RBepPB0Ub9Fq4icHAAyqA?e=2127196800&token=q6iZ0sQtf9U7s1qz0r4yMawNq3-u2w6lbnai6y2J:AoJZ9VINr-qkJk0CIgR84nxb2r8=)

雷哥

2026 年 04 月 08 日 10:10

这个方法太牛逼了，简直是做一切产品原型、深度调研，做 skills 或者 AI native 的一个神器。

# 需求从哪来

所有的需求其实来源于深度对话的挖掘。深度对话挖掘是跟我们要做产品的受访者来进行的。比如说企业来讲，它内部可能有一个提需求的部门；如果对外，很可能是我们对不同人的采访。这些采访未来都会形成我们产品初步的原型想法和构建思路。

## 以前怎么做

以前我们做这个事情，是通过调研、采访问题，然后进行整理，再转到开发去进行开发。

## 现在怎么做

但实际上现在不用了。现在我们需要有一个非常牛逼的录音笔或录音设备。这个设备录完之后可以无线传输，超长待机。我们拿这个录音设备去跟用户，基于某个想法或者需求，进行深度的自由对话，把这些对话都录下来。

每次我们对某些想法都会有一些深入的探索。以前用笔去记录的话，这些东西都会有一些信息损失。如果我们拿录音设备去跟用户做真实的访谈和需求深入调研，就会发现这些录音转成文字之后，就会形成我们的原始文档，而这些文档就会成为我们产品原型和 skills 的核心理念。

![](https://article-images.zsxq.com/FkDf0RO4iHZNTQqqbu91Sg6YvFhE)

  

## 我已经验证过了

我为什么和大家讲这件事情？因为我已经把给企业做的一些 skills，通过语音对话采访的方式，真实地做成了一个一个的 skills。这些 skills 都来源于我跟用户最真实的访谈内容，它保证了信息的不丢失。

## 具体做法

具体的做法，就是先抱着一个探索的心态去和用户聊。聊完之后，有一些东西不清晰的话，我们再整理，再跟用户去聊。聊很多次之后，你会有五六个录音。这五六个录音已经让你把 skills 或者要做的产品想得非常清晰了。结合这些清晰的想法，和我之前给大家讲过的 lovable 原型，或者说 Skills Creator，结合这些东西去详细设计，就可以开发出一个 skills 了。包括如果你在做一款真正的产品，结合我给大家讲的 AI 自动化 coding，它就可以变成一个 feature 一个 feature 地慢慢自动化 coding 出来了。

![](https://article-images.zsxq.com/FquONfMgobyFu6FTYEnKppnEPux-)

  

## 为什么这是优势

这是非常快速的产品做法，也是咱们这个社群里边非常有优势的地方。因为大家多多少少对程序都有一些理解，对产品都有一些深入的探索，再结合这样的方式去做，你就拥有非常强大的先发优势。

这个方法太牛逼了，简直是做一切产品原型、深度调研，做 skills 或者 AI native 的一个神器。

# 需求从哪来

所有的需求其实来源于深度对话的挖掘。深度对话挖掘是跟我们要做产品的受访者来进行的。比如说企业来讲，它内部可能有一个提需求的部门；如果对外，很可能是我们对不同人的采访。这些采访未来都会形成我们产品初步的原型想法和构建思路。

## 以前怎么做

以前我们做这个事情，是通过调研、采访问题，然后进行整理，再转到开发去进行开发。

## 现在怎么做

但实际上现在不用了。现在我们需要有一个非常牛逼的录音笔或录音设备。这个设备录完之后可以无线传输，超长待机。我们拿这个录音设备去跟用户，基于某个想法或者需求，进行深度的自由对话，把这些对话都录下来。

每次我们对某些想法都会有一些深入的探索。以前用笔去记录的话，这些东西都会有一些信息损失。如果我们拿录音设备去跟用户做真实的访谈和需求深入调研，就会发现这些录音转成文字之后，就会形成我们的原始文档，而这些文档就会成为我们产品原型和 skills 的核心理念。

![](https://article-images.zsxq.com/FkDf0RO4iHZNTQqqbu91Sg6YvFhE)

  

## 我已经验证过了

我为什么和大家讲这件事情？因为我已经把给企业做的一些 skills，通过语音对话采访的方式，真实地做成了一个一个的 skills。这些 skills 都来源于我跟用户最真实的访谈内容，它保证了信息的不丢失。

## 具体做法

具体的做法，就是先抱着一个探索的心态去和用户聊。聊完之后，有一些东西不清晰的话，我们再整理，再跟用户去聊。聊很多次之后，你会有五六个录音。这五六个录音已经让你把 skills 或者要做的产品想得非常清晰了。结合这些清晰的想法，和我之前给大家讲过的 lovable 原型，或者说 Skills Creator，结合这些东西去详细设计，就可以开发出一个 skills 了。包括如果你在做一款真正的产品，结合我给大家讲的 AI 自动化 coding，它就可以变成一个 feature 一个 feature 地慢慢自动化 coding 出来了。

![](https://article-images.zsxq.com/FquONfMgobyFu6FTYEnKppnEPux-)

  

## 为什么这是优势

这是非常快速的产品做法，也是咱们这个社群里边非常有优势的地方。因为大家多多少少对程序都有一些理解，对产品都有一些深入的探索，再结合这样的方式去做，你就拥有非常强大的先发优势。

这个方法太牛逼了，简直是做一切产品原型、深度调研，做 skills 或者 AI native 的一个神器。

# 需求从哪来

所有的需求其实来源于深度对话的挖掘。深度对话挖掘是跟我们要做产品的受访者来进行的。比如说企业来讲，它内部可能有一个提需求的部门；如果对外，很可能是我们对不同人的采访。这些采访未来都会形成我们产品初步的原型想法和构建思路。

## 以前怎么做

以前我们做这个事情，是通过调研、采访问题，然后进行整理，再转到开发去进行开发。

## 现在怎么做

但实际上现在不用了。现在我们需要有一个非常牛逼的录音笔或录音设备。这个设备录完之后可以无线传输，超长待机。我们拿这个录音设备去跟用户，基于某个想法或者需求，进行深度的自由对话，把这些对话都录下来。

每次我们对某些想法都会有一些深入的探索。以前用笔去记录的话，这些东西都会有一些信息损失。如果我们拿录音设备去跟用户做真实的访谈和需求深入调研，就会发现这些录音转成文字之后，就会形成我们的原始文档，而这些文档就会成为我们产品原型和 skills 的核心理念。

![](https://article-images.zsxq.com/FkDf0RO4iHZNTQqqbu91Sg6YvFhE)

  

## 我已经验证过了

我为什么和大家讲这件事情？因为我已经把给企业做的一些 skills，通过语音对话采访的方式，真实地做成了一个一个的 skills。这些 skills 都来源于我跟用户最真实的访谈内容，它保证了信息的不丢失。

## 具体做法

具体的做法，就是先抱着一个探索的心态去和用户聊。聊完之后，有一些东西不清晰的话，我们再整理，再跟用户去聊。聊很多次之后，你会有五六个录音。这五六个录音已经让你把 skills 或者要做的产品想得非常清晰了。结合这些清晰的想法，和我之前给大家讲过的 lovable 原型，或者说 Skills Creator，结合这些东西去详细设计，就可以开发出一个 skills 了。包括如果你在做一款真正的产品，结合我给大家讲的 AI 自动化 coding，它就可以变成一个 feature 一个 feature 地慢慢自动化 coding 出来了。

![](https://article-images.zsxq.com/FquONfMgobyFu6FTYEnKppnEPux-)

  

## 为什么这是优势

这是非常快速的产品做法，也是咱们这个社群里边非常有优势的地方。因为大家多多少少对程序都有一些理解，对产品都有一些深入的探索，再结合这样的方式去做，你就拥有非常强大的先发优势。

![](https://articles.zsxq.com/assets_dweb/logo@2x.png)

知识星球

![](data:image/png;base64,)

扫码加入星球

查看更多优质内容

https://wx.zsxq.com/mweb/views/joingroup/join_group.html?group_id=28882285418421