...
Apache Paimon 是一个高吞吐、低延迟的流式数据湖，专注于实时数据更新与高效查询。它由 Apache Flink 社区孵化，旨在解决传统数据湖在实时数据处理中的痛点，如数据更新延迟、流批分离架构复杂等问题，Paimon最大优势是，可以有效解决现代大数据系统面临的三大核心挑战：平衡低延迟与成本、简化复杂的数据生产架构，以及无缝集成 AI 场景。Apache Paimon可部署在S3、HDFS等系统上，支持通过Flink SQL实时处理数据，并支持SparkSQL任务进行批处理，并且支持通过Doris、Presto等OLAP引擎来直接查询。
