在 Apache Paimon 中，主键表通过 **Merge Engine（合并引擎）** 来处理具有相同主键的多条记录，将它们合并成一条记录以保持主键的唯一性[3][4]。
...
* **特殊要求**：为了在数据湖中生成和跟踪数据的变更日志，该引擎通常必须与特定的 Changelog Producer（如 `lookup` 或 `full-compaction`）配合使用[1][2][10]。
...
在 Apache Paimon 中，配置 Merge Engine（合并引擎）主要是通过在建表语句（如 Flink SQL）的 `WITH` 参数中指定 `'merge-engine'` 属性来实现的。创建 Paimon 表时，必须指定主键（`PRIMARY KEY`），因为合并机制是基于主键来触发的[1]。
