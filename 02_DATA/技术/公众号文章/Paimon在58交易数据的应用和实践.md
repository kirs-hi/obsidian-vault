...
上图展示了之前的[[实时数仓]]计算架构。前面从 ODS 至 APP 的部分是实时链路，主要由 Flink、Kafka 和 HBase、MySQL 组成。此部分链路中的数据来自在线系统生成的交易数据日志。数据经过 WMB（公司自研的消息队列系统），使用 Flink Streaming 对其进行自定义的数据转换操作，将数据下发到 Kafka，后续流程使用 [[Flink SQL 完整语法教程|Flink SQL]] 构建实时数仓，最终结果落在 MySQL 中，供下游系统使用。
...
[[paimon|Paimon]] 是一个实时[[数据湖项目|数据湖]]格式，具备以下核心功能：

- **实时更新**：基于 LSM 树结构，支持主键表的高效流式更新（如去重、部分更新、聚合），并提供变更日志（Changelog）生成能力，简化流式分析。
- **流批一体**：统一支持流式与批处理操作，兼容 Flink、[[spark|Spark]]、Hive 等引擎，实现数据湖的实时写入与离线分析。
...
- **资源消耗大**：需要全局排序，Shuffle 数据量巨大；内存占用高，尤其在窗口较大或[[数据倾斜实践|数据倾斜]]严重时；Checkpoint 文件体积膨胀，影响任务稳定性
...
在原有的实时[[数仓架构]]中，Kafka 被用作中间消息队列，承担数据采集、缓冲和分发的角色。
