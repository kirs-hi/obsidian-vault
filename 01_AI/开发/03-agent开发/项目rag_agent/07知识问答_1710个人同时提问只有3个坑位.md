...
假设你在一家在线教育公司做开发，公司前段时间上了 [[06-RAG|RAG]] 智能客服，主要做课程咨询、退费政策、报名引导这类问题。日常流量不大，平均下来一台机器同时跑 5 ~ 10 个请求。但每周三下午 3 点，运营会推一个「限时五折」的活动短信，全网用户瞬间涌进来问优惠详情、能不能叠加优惠券、报名截止时间。促销那一刻同时来了 200 个 RAG 请求。

这时候问题就来了：

- 下游模型服务有并发上限——本地部署的 GPU 显存有限，同时跑几十个推理任务直接 OOM；在线 API 平台（如 SiliconFlow）也有并发限制，一下涌进 200 个请求，绝大多数会直接报 `429 Too Many Requests`

- Tomcat 默认 200 个工作线程会被这些 [[01基础_20SSE协议与流式响应|SSE]] 长连接占满——单个 RAG 请求要跑 30 秒到 1 分钟，普通的 REST 接口都进不来了
