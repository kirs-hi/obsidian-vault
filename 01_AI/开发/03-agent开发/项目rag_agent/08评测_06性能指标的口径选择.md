...
对话产品的性能度量跟传统 API 有一个本质区别：用户不需要等完整回答出来才知道系统有没有响应。[[01基础_20SSE协议与流式响应|SSE]] 流式输出下，第一个字蹦出来的那一刻，用户的等待焦虑就解除了——后面的字像打字机一样逐个出现，体感是流畅的。所以性能指标的核心不是整流耗时，而是首字延迟——TTFT（Time To First Token）。
...
TTFT 覆盖的是从请求发出到答案首字出现的这段时间——Query 改写、[[01基础_18意图识别与多路由调度策略|意图识别]]、[[01基础_10向量检索策略与召回优化|向量检索]]、[[06调度引擎_08Rerank重排序与辅助工具|Rerank]]、LLM 生成第一个 token，这些串行步骤全算在里面。这条链路走下来，5 秒左右的 TTFT 是常态。后面的 token 逐个输出只是增加了整流耗时，不影响用户对响应速度的感知。
...
自建指标到这里全部讲完了。它们能算命中没命中、排第几、路由对不对、等了多久——但算不了内容对不对。召回的 chunk 都对，模型却胡编了一段，自建指标完全无感。这种语义级的评估需要另一套工具——RAGAS，一个全靠 LLM-as-judge 驱动的 [[06-RAG|RAG]] 评估框架。它怎么工作？几十个指标为什么只挑 5 个？跑一次要花多少钱？
