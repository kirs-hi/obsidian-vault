...
上一篇讲完检索策略，咱们已经能通过混合检索 + [[06调度引擎_08Rerank重排序与辅助工具|重排序]]拿到高质量的 Top-K chunk 了。到这一步，很多人觉得 [[06-RAG|RAG]] 的核心问题已经解决了——检索到了对的内容，大模型自然能给出对的答案。
...
经验法则：[[理论学习_System_Prompt_如何设计_|System Prompt]] 控制在 5~8 条核心规则，每条规则聚焦一个明确的行为约束。
...
这种情况可以通过 Query 改写（Query Rewriting）来优化，比如把“退货怎么弄”改写成“请问退货的具体流程和条件是什么”。但 Query 改写本身是一个独立的话题，涉及到[[01基础_16多轮对话记忆设计|多轮对话]]上下文理解、[[01基础_18意图识别与多路由调度策略|意图识别]]等，这一篇先不展开，后续篇章再详细讲。
...
解析逻辑很简单——用[[14-正则表达式|正则表达式]]提取 `[数字]` 模式：
...
| [[01基础_06数据分块Chunk策略与实践|数据分块]] | 把长文本切成适合检索的 chunk | Chunking 篇 |
| 元数据管理 | 给 chunk 贴标签（来源、权限、时间等） | Metadata 篇 |
| 向量化 | 把文本 chunk 转成数字向量 | [[01基础_08从文本到向量之理解Embedding|Embedding]] 篇 |
| [[01基础_09向量数据库的原理与选型|向量数据库]] | 存储向量，支持 ANN 检索 | Milvus 篇 |
| 混合检索 + 重排序 | [[01基础_10向量检索策略与召回优化|向量检索]] + BM25 + Reranker 精排 | 检索策略篇 |
