...
公司上了一套 [[06-RAG|RAG]] 系统，员工可以直接问问题，系统从知识库里检索相关内容并生成回答。听起来很美好，但实际跑起来，问题一个接一个冒出来。
...
这些元数据会和文本内容一起，被送到[[01基础_09向量数据库的原理与选型|向量数据库]]里存储。后续检索的时候，不仅可以根据文本相似度找到相关的块，还可以根据元数据做过滤、排序、引用生成等操作。
...
- **[[01基础_06数据分块Chunk策略与实践|Chunk]] 级标注** ：系统自动生成 chunk 级的元数据（如 `chunk_index` 、 `start_offset` ）
...
元数据设计完成后，每个 chunk 就从“一段文本“变成了“一段带标签的文本”。但这些文本还是人类能读懂的自然语言，计算机要做相似度检索，需要把它们转成数字表示——这就是向量化（[[01基础_08从文本到向量之理解Embedding|Embedding]]）。
