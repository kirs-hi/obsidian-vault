...
上一篇走完了 [[理论学习_MCP_协议与开放工具生态|MCP]] 工具调用的完整链路——意图分流 → 注册表查找 → 参数提取 → 远程执行 → 结果格式化 → 汇入主流水线。但链路中有一步被当黑盒跳过了：参数提取。
...
两个方法，区别在于第二个多了一个 `customPromptTemplate` 参数。第 12 篇讲过，意图节点上有一个 `paramPromptTemplate` 字段，允许为特定工具定制参数提取的[[提示词]]。这个定制化能力就是通过第二个方法传进来的——有自定义提示词就用自定义的，没有就走默认模板。
...
[[理论学习_System_Prompt_如何设计_|System Prompt]] 定义了参数提取的完整规则，模板文件是 `prompt/mcp-parameter-extract.st` ：
...
但故事还没讲完。KB 检索出来了 5 条 [[01基础_06数据分块Chunk策略与实践|Chunk]]，MCP 工具拿回来了一份销售报告——这些上下文现在分别躺在 `RetrievalContext` 的 `kbContext` 和 `mcpContext` 两个字段里。下一步要把它们组装成最终发给大模型的 Prompt。System Prompt 怎么选？Evidence 放在对话历史的什么位置？多个子问题的上下文怎么编号？下一篇来讲 Prompt 组装。
