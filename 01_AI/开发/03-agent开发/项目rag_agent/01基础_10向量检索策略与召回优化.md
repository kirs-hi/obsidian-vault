...
上一篇讲完[[01基础_09向量数据库的原理与选型|向量数据库]]，咱们已经能用 Milvus 做向量检索了——把用户的问题转成向量，在 Milvus 里找最相似的 chunk，看起来很完美。但实际跑起来，你会发现有些场景下纯向量检索会翻车。
...
| 对同义词的处理 | 不行。“手机”和“移动电话”匹配不上 | 行。[[01基础_08从文本到向量之理解Embedding|Embedding]] 模型能把同义词映射到相近的向量 |
| 对专有名词的处理 | 很强。“iPhone 16 Pro Max”能精确匹配 | 一般。可能被拆成“iPhone16”“Pro”“Max”分别理解 |
| 计算成本 | 低。只需要统计词频，不需要调用模型 | 高。需要调用 Embedding 模型把 query 转成向量 |
| 典型应用 | 搜索引擎（Google、Elasticsearch） | 语义搜索、推荐系统、[[06-RAG|RAG]] |
...
| [[01基础_06数据分块Chunk策略与实践|Chunk]] | 向量检索排名 | 关键词检索排名 | RRF 分数（k=60） | 计算过程 |
| --- | --- | --- | --- | --- |
...
| 混合检索 + [[06调度引擎_08Rerank重排序与辅助工具|Rerank]] | iPhone 16 Pro Max 退货政策 | 七天无理由退货说明 | 拆封商品退货规则 |
