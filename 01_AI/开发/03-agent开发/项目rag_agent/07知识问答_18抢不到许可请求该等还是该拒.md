...
- **等了 20 秒还没轮到自己怎么办？** ——总不能把 [[01基础_20SSE协议与流式响应|SSE]] 连接挂着不响应，得给用户一个明确的交代。但只关连接也太粗暴了，前端只能看到一个空白
...
为什么 lease 设 600 秒？要比业务最长耗时大一些。[[06-RAG|RAG]] 业务最长可能跑 1~2 分钟（深度思考 + 长文档生成 + 重试）。600 秒 = 10 分钟，留了 5 倍的安全余量。
...
| 会话记忆 | 2~3 | 大模型没有记忆，[[01基础_16多轮对话记忆设计|多轮对话]]怎么不失忆 | 让连续追问保留上下文 |
| [[01基础_17查询重写与语义增强机制|查询重写]] | 4 | 用户说的话和该搜的词不是一回事 | 把口语化问题变成检索系统能用的 query |
| [[01基础_18意图识别与多路由调度策略|意图识别]]树 | 5~9 | 四分类撑不住 20 个知识库，怎么精准路由 | 让模糊问题被分发到对的知识库 |
| 多通道检索 | 10~11 | 一次提问同时查多个库，最后只给模型几条 | 召回 + 精排，过滤噪声 |
| [[理论学习_MCP_协议与开放工具生态|MCP]] 工具调用 | 12~13 | 知识库答不了的问题交给工具去查 | 扩展模型的能力边界到外部系统 |
