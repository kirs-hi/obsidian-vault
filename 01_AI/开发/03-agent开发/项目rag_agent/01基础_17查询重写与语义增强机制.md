...
上一篇讲了会话记忆，你的 [[06-RAG|RAG]] 系统终于能知道之前聊了什么了。用户先问“iPhone 16 Pro 的退货政策是什么”，再追问“那它的保修期呢”，模型看到了完整的对话历史，知道“它”指的是 iPhone 16 Pro，回答没问题。

但别高兴得太早。

RAG 系统在回答之前，要先去[[01基础_09向量数据库的原理与选型|向量数据库]]检索相关的 chunk。检索系统拿到的 query 是什么？是用户的原始问题——那它的保修期呢。
...
指代消解，说白了就是把代词替换成它指代的具体实体。这是[[01基础_16多轮对话记忆设计|多轮对话]]中最高频的改写场景。
...
这种改写主要对\*\*关键词检索（BM25）\*\*有帮助——BM25 是按词匹配的，同义词扩展能提高命中率。对[[01基础_10向量检索策略与召回优化|向量检索]]来说，帮助有限，因为向量检索本身就能理解语义相似性（屏幕碎了和屏幕破损的向量已经很接近了）。
...
比如用户说“今天天气怎么样”，这明显不是一个知识库检索的问题；用户说“帮我查一下我的年假余额”，这应该调 [[01基础_12理解函数调用Function Call|Function Call]]；用户说“iPhone 16 Pro 的退货政策是什么”，这才需要走 RAG 检索。

下一篇咱们来聊 **[[01基础_18意图识别与多路由调度策略|意图识别]]与问题路由** ——让 RAG 系统在收到用户消息后，自动判断应该走哪条路：知识库检索、工具调用、还是直接对话，做出正确的路由决策。
