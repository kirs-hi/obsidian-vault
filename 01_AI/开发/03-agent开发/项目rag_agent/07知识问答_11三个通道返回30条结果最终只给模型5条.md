...
文章最后留了一个问题：定向检索从两个 Collection 搜回了 14 条，全局检索从 5 个 Collection 搜回了 15 条，加起来 29 条 [[01基础_06数据分块Chunk策略与实践|Chunk]]——能直接塞进 LLM 的 Prompt 吗？

> V1 版本 Ragent AI 代码未接入关键词检索，但是文章预留了关键词检索通道进行讲解。

答案是不能。把场景拉回电商客服。用户问了一句“AirPods Pro 怎么退货？”，经过[[01基础_18意图识别与多路由调度策略|意图识别]]和多通道并行检索后，三个通道各自返回了结果：
...
- **全局[[01基础_10向量检索策略与召回优化|向量检索]]通道** （ `VECTOR_GLOBAL` ）：在所有 Collection 中做向量检索，返回 10 条 Chunk，分数范围 0.78~0.35
...
- **`context`** ：全局检索上下文，携带 `topK` 、用户问题等参数。精排处理器需要 `context.getMainQuestion()` 作为 [[06调度引擎_08Rerank重排序与辅助工具|Rerank]] 的 query，需要 `context.getTopK()` 决定截断数量
...
> 咱们在 [[06-RAG|RAG]] 基础系列检索策略已经了解过 Bi-Encoder 和 Cross-Encoder 的区别，这里从工程视角简要回顾。
...
到这里 KB 检索链路就完整了——意图识别告诉系统该查哪个知识库，多通道检索把文档捞出来，后处理流水线把结果精炼到 5 条。但 KB 检索不是万能的。用户问“我的订单到哪了？”，知识库里没有这个用户的实时订单数据，靠检索文档回答不了。第 10 篇提到过 [[理论学习_MCP_协议与开放工具生态|MCP]] 意图和 KB 意图是并行处理的，那 MCP 工具调用链路是怎么跑的？工具定义怎么注册？参数怎么传？调用结果怎么拼回大模型的上下文？
