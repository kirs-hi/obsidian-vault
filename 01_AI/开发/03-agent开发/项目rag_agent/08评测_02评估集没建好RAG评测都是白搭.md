...
这是整个 schema 里最重要的一个布尔字段。 `true` = 这条问题应该走 [[06-RAG|RAG]] 检索， `false` = 不应该检索，应该走系统兜底（直接回复或转人工）。
...
| --- | --- | --- |
| `competitor_boundary` | 提到竞品，应拒答 | 拒答陷阱没拦住，[[01基础_18意图识别与多路由调度策略|意图识别]]或 prompt 需要加边界 |
...
- **Runner 阶段** ：150 条串行调两个接口（[[01基础_20SSE协议与流式响应|SSE]] + JSON），每条等 LLM 生成完，全跑下来要几十分钟
