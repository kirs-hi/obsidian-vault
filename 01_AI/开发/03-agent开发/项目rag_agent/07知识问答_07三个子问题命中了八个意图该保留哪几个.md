...
这篇文章要回答的就是这个问题。 `IntentResolver` 作为[[01基础_18意图识别与多路由调度策略|意图识别]]阶段的编排者，负责把子问题分发出去并行跑，把结果收回来做总量封顶，保证既不丢子问题，又不让意图数量爆炸。
...
为什么要隔离？意图分类是网络 IO 密集型任务（调用外部 LLM 服务），和检索（调用[[01基础_09向量数据库的原理与选型|向量数据库]]）、记忆加载的 IO 特征不同。如果所有异步任务共用一个线程池，检索慢了（比如向量数据库偶尔抖动）可能把线程池占满，导致意图分类也卡住——明明子问题 LLM 调用已经返回了，拿不到线程执行后续逻辑。
...
> 注意： `resolve()` 方法本身会阻塞当前线程（通常是 [[06-RAG|RAG]] pipeline 的主线程），但意图分类的 LLM 调用在 `intentClassifyExecutor` 池里异步执行，不占主线程资源。
...
| 子问题 | 命中意图 | 分数 | 类型 |
| --- | --- | --- | --- |
| A | 订单查询 | 0.92 | [[理论学习_MCP_协议与开放工具生态|MCP]] |
| A | 订单详情 | 0.58 | KB |
| B | 退换货政策 | 0.88 | KB |
| B | 售后服务 | 0.62 | KB |
