...
问题在于，Ragent 的生产接口 `/rag/v3/chat` 是 [[01基础_20SSE协议与流式响应|SSE]] 流式输出，一个字一个字蹦出来——拿得到答案和首字耗时，但拿不到中间产物。召回了哪些 chunk、分到了哪个意图，SSE 流里没有这些数据。
...
三行核心调用——改写拆分 → [[01基础_18意图识别与多路由调度策略|意图识别]] → 检索引擎——跟生产链路用的是同一套 Service。区别在于拿到检索结果就打包返回了，不调大模型、不走 SSE。
...
| `hasKb` / `hasMcp` | 是否走了 KB 检索 / [[理论学习_MCP_协议与开放工具生态|MCP]] 工具 |
| `latencyMs` | 接口耗时 |
...
| `difficulty` / `requires_rag` | 难度和是否需要 [[06-RAG|RAG]] |
