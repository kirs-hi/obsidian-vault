...
上一篇讲了 Query 改写，[[06-RAG|RAG]] 系统的[[01基础_16多轮对话记忆设计|多轮对话]]能力已经比较完整了：会话记忆让模型记住了上下文，Query 改写让检索系统也能理解上下文，检索效果大幅提升。

但你把这套系统上线之后，会发现一个尴尬的问题。

用户说“你好，我想咨询一下”，系统拿“你好我想咨询一下”去[[01基础_09向量数据库的原理与选型|向量数据库]]里检索，召回了退货政策、保修条款、配送时效三个 chunk，然后模型基于这些 chunk 回答：“您好，关于退货政策，我们提供七天无理由退货服务……”——用户只是打了个招呼，你给人家讲退货政策？
...
| 工具调用（查我的订单、我还剩几天年假） | 知识库里没有用户的个人数据，检索不到有用信息，模型说“找不到相关信息” | 调 [[01基础_12理解函数调用Function Call|Function Call]] / [[理论学习_MCP_协议与开放工具生态|MCP]] 查实时数据 |
...
处理方式：走完整的 RAG 检索流程——Query 改写 → [[01基础_10向量检索策略与召回优化|向量检索]] → [[06调度引擎_08Rerank重排序与辅助工具|重排序]] → 生成答案。
...
处理方式：模型直接回答，不走检索，也不调工具。[[理论学习_System_Prompt_如何设计_|System Prompt]] 里定义好闲聊时的回复风格就行。
...
最简单直接的方案：用关键词和[[14-正则表达式|正则表达式]]判断意图。
