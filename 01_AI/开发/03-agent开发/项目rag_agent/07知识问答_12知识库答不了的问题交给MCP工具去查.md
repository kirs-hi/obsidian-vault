...
上一篇拆解了后处理流水线——三个通道返回的约 30 条原始 [[01基础_06数据分块Chunk策略与实践|Chunk]]，经过去重、Cross-Encoder 精排、topK 截断，最终只留 5 条高质量上下文喂给大模型。到那里为止，KB 检索链路就完整收尾了：[[01基础_18意图识别与多路由调度策略|意图识别]]告诉系统该查哪个知识库，多通道检索把文档捞出来，后处理流水线把结果精炼到 5 条。
...
要回答这类问题，得让系统实时去业务系统查。这就是 [[理论学习_MCP_协议与开放工具生态|MCP]]（Model Context Protocol，模型上下文协议）工具调用要解决的事。

本篇不讲 MCP 协议本身是什么——基础系列已经讲过 [[01基础_12理解函数调用Function Call|Function Call]] 和 MCP 的概念，MCP 系列也有专门的协议规范和 SDK 架构拆解。本篇只聚焦一件事： **在 Ragent 这个系统里，一次 MCP 工具调用从被识别到被执行再到结果回流，完整链路是怎么跑的。**
...
| `paramPromptTemplate` | 自定义参数提取[[提示词]]（可选） | 针对特定工具的定制提取逻辑 |
...
**阶段 8** ： `RetrievalContext.hasMcp()` 返回 `true` ，Pipeline 选择 MCP 相关的 [[理论学习_System_Prompt_如何设计_|System Prompt]] 模板， `temperature=0.3` ，大模型基于这份实时数据生成最终答案。
