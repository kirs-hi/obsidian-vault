...
`bootstrap` 是最上层的业务模块，它依赖 `framework` （通用基础设施）和 `infra-ai` （AI 能力层）。 `mcp-server` 是一个独立的 Spring Boot 应用，通过 HTTP 提供 [[理论学习_MCP_协议与开放工具生态|MCP]] 工具调用服务。
...
- **`RetrievedChunk`** ：[[01基础_10向量检索策略与召回优化|向量检索]]返回的单条结果，包含 `id` 、 `text` 和相似度 `score` 。
...
这个模块是整个系统和大模型之间的桥梁。它屏蔽了不同 AI 服务商的接口差异，对上层提供统一的调用方式。内部按能力分为四个子包： `chat` （对话）、 `embedding` （向量化）、 `rerank` （[[06调度引擎_08Rerank重排序与辅助工具|重排序]]）、 `model` （路由与容错）。
...
- **`FirstPacketAwaiter`** ：等待[[01基础_20SSE协议与流式响应|流式响应]]的第一个数据包到达，用于超时检测。
...
- 3.
	**[[01基础_18意图识别与多路由调度策略|意图识别]]** —— `IntentResolver` 把问题匹配到意图树节点，判断该走知识库检索还是 MCP 工具调用。
...
| `IndexerNode` | 调用 [[01基础_08从文本到向量之理解Embedding|Embedding]] 服务向量化，写入 Milvus |
...
前端工程，技术栈是 React 18 + [[Day2-JavaScript和TypeScript|TypeScript]] + Vite + [[Day3-Tailwind和shadcn|Tailwind]] CSS，UI 组件基于 Radix UI。这篇文档主要聚焦后端，前端不再展开。
...
| `resources/docker/` | Docker Compose 配置，包含 Milvus [[01基础_09向量数据库的原理与选型|向量数据库]]的部署文件 |
