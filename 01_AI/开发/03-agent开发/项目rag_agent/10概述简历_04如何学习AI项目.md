...
Ragent AI 覆盖了 [[06-RAG|RAG]] 从数据入库到问答生成的完整链路，模块多、设计细节也多。直接翻源码大概率迷失在各种类和接口之间。这篇文档帮你理清学习节奏，少走弯路。
...
- **AI 辅助理解** ：遇到看不懂的模块，直接把代码贴给 AI 工具。比如把熔断器的状态转换逻辑贴进去，让它解释状态流转条件。Cursor、[[Claude Code 命令与最佳实践|Claude Code]]、Codex 都行，选顺手的
...
| 5 | 数据分块 [[01基础_06数据分块Chunk策略与实践|Chunk]] | 为什么要分块，五种分块策略各自适合什么场景 |
| 6 | 元数据管理 Metadata | 元数据在检索过滤、答案溯源中的作用 |
| 7 | 向量化 [[01基础_08从文本到向量之理解Embedding|Embedding]] | 文本怎么变成向量，余弦相似度怎么算 |
| 8 | [[01基础_09向量数据库的原理与选型|向量数据库]] VectorDatabase | Milvus 核心概念，IVF 和 HNSW 两种索引算法 |
| 9 | 检索策略 RetrievalStrategy | BM25、混合检索、RRF 融合、Reranking 重排序 |
| 10 | 生成策略 GenerationStrategy | 三段式 Prompt 结构，幻觉抑制，引用对齐 |
| 11 | 函数调用 FunctionCall | [[01基础_12理解函数调用Function Call|Function Call]] 的本质和完整流程 |
| 12 | [[理论学习_MCP_协议与开放工具生态|MCP]] 协议 | MCP 三层架构，工具调用的标准化方案 |
| 13 | 会话记忆 ConversationMemory | 五种记忆策略，Token 预算分配 |
| 14 | Query 改写 QueryRewrite | 指代消解、上下文补全、口语化转正式 |
| 15 | [[01基础_18意图识别与多路由调度策略|意图识别]]与问题路由 IntentRouting | 四种意图类型，规则 + 大模型的混合分类方案 |
...
- [[06调度引擎_08Rerank重排序与辅助工具|Rerank]] 重排序与辅助工具

**SSE 系列** —— [[01基础_20SSE协议与流式响应|流式响应]]的协议和工程实现
...
`IntentDirectedSearchChannel` 根据意图识别结果做定向检索，只在命中意图关联的知识库范围内搜索； `VectorGlobalSearchChannel` 做全局[[01基础_10向量检索策略与召回优化|向量检索]]，不限定范围。两个通道通过 `CompletableFuture` 并行执行，提交到专用的 `ragRetrievalExecutor` 线程池。
...
[[01基础_16多轮对话记忆设计|多轮对话]]的记忆管理要在 Token 成本、响应速度和上下文质量之间做平衡。
...
`MCPParameterExtractor` 根据工具定义（ `MCPTool` ）里声明的参数 Schema，用 LLM 从用户问题中提取参数。意图节点上还可配置 `paramPromptTemplate` 自定义提取[[提示词]]。
