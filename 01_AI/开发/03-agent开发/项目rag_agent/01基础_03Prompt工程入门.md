...
- [[06-RAG|RAG]] 场景下的 Prompt 有特殊性（限定知识来源、防止编造、要求引用），但底层原理是通用的
...
你的检索系统把这段内容当作普通 chunk 返回，模型误把 chunk 中的指令当作真实指令执行，就会输出系统[[提示词]]。这就是 Prompt 注入攻击。
...
首先定义 [[01基础_06数据分块Chunk策略与实践|Chunk]] 数据结构：
...
- 方案二更灵活，适合需要动态调整参考资料和问题的场景（如[[01基础_16多轮对话记忆设计|多轮对话]]）
