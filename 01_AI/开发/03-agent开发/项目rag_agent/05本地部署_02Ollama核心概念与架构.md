...
关掉终端窗口，模型还在不在？跑着呢还是停了？你的 Java 项目要怎么跟它对接？ `ollama serve` 和 `ollama run` 到底啥关系？模型文件存哪了？同时跑 Chat 和 [[01基础_08从文本到向量之理解Embedding|Embedding]] 两个模型会不会互相抢显存？别人文章里写的 `OLLAMA_KEEP_ALIVE=-1` 又是控制什么的？
...
设定模型的默认推理参数。几个 [[06-RAG|RAG]] 场景下特别重要的参数：
...
Chat Template 控制的是[[01基础_16多轮对话记忆设计|多轮对话]]的格式化方式——怎么把 system、user、assistant 这些角色的消息拼成模型能理解的文本。用的是 Go template 语法。
...
`stream` 设为 `true` 时走流式输出，返回一连串 JSON 对象，每个包含一小段增量文本——跟你之前学的 [[01基础_20SSE协议与流式响应|SSE]] 协议是同一个套路。
...
一个判断标准：如果你的服务只有你自己和几个同事在用、挂了手动重启也能接受，Ollama 可以顶。如果你的服务面向真实用户、需要 [[sla 治理|SLA]] 保障、需要监控报警、需要弹性扩缩容，Ollama 不行，上 vLLM。
...
- **SGLang** ：[[07-Agent|Agent]] 场景的前缀复用优化
