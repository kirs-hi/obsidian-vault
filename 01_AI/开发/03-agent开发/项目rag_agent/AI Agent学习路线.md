---
title: "AI Agent学习路线 | 拿个offer · Java&AI 实战圈"
source: "https://nageoffer.com/ai/"
author:
published: 2026-04-21
created: 2026-06-05
description: "AI工程师的角色定义、技能要求以及与算法工程师的区别。本文旨在帮助程序员了解AI工程师的职责和技能需求，消除对AI领域的误解，并提供清晰的学习路径，助力程序员顺利转型为AI工程师。"
tags:
  - "clippings"
---
作者：程序员马丁
...
> note
> 
> [Ragent AI](https://nageoffer.com/ragent) —— 从 0 到 1 纯手工打造企业级 Agentic [[06-RAG|RAG]]，拒绝 Demo 玩具！AI 时代，助你拿个offer。
...
> 从本月起， [https://github.com/nageoffer/awesome-ai-handbook](https://github.com/nageoffer/awesome-ai-handbook) 正式推出 AI [[07-Agent|Agent]] [[面试题]]解析内容，全面发力 AI 赛道，助力求职者把握前沿技术机会。
...
- **[[01基础_08从文本到向量之理解Embedding|Embedding]] Model（嵌入模型）** ：把文本转成向量（一串数字），用于语义搜索和相似度计算，后面讲 RAG 时会用到
...
- **[[01基础_12理解函数调用Function Call|Function Calling]] / Tool Use（函数调用）** ：让模型在回答过程中调用工具——比如查数据库、调天气接口。模型并不真正执行代码，而是返回结构化的调用意图（调哪个函数、传什么参数），由你的程序去执行并把结果喂回模型。这是构建 Agent 的基础能力，后面会展开讲
...
- **Structured Output / JSON Mode（结构化输出）** ：让模型按指定的 JSON Schema 返回结构化数据，而不是自由文本。对接下游系统时非常实用——不用再费劲从自然语言里解析数据。现在随着模型越来越完善，基于[[提示词]]也能达到类似效果
- **流式输出（[[01基础_20SSE协议与流式响应|SSE]] / Streaming）** ：模型一边生成一边返回，用户不用等全部生成完才看到结果。就是 ChatGPT 那种逐字蹦出来的效果（打字机）。技术上基于 Server-Sent Events 协议
...
- **[[01基础_09向量数据库的原理与选型|向量数据库]]（Vector Store）** ：专门存储和检索向量的数据库。主流选项：
...
- **Reranker（[[06调度引擎_08Rerank重排序与辅助工具|重排序]]模型）** ：初步检索可能返回几十条结果，用一个专门的重排序模型对它们做精细打分，把最相关的排到前面。相当于先粗筛再精选，显著提升最终质量
...
- **Graph RAG** ：用知识图谱替代（或补充）[[01基础_10向量检索策略与召回优化|向量检索]]，更擅长处理实体关系和多跳推理类问题（比如“张三的上级的部门负责什么业务”）
...
Prompt 技巧贯穿 AI 应用的所有环节（写 RAG 的 [[理论学习_System_Prompt_如何设计_|System Prompt]]、设计 Agent 的指令都要用到），但在 Agent 场景下最为关键和复杂，所以放在这里集中介绍。
...
- **[[理论学习_MCP_协议与开放工具生态|MCP]]（Model Context Protocol）** ：Anthropic 提出的开放协议，为模型连接外部工具和数据源定义了统一标准。你可以把它理解成 **AI 世界的 USB-C 接口** ——工具提供方只需实现一次 MCP Server，所有支持 MCP 的客户端（Cursor、Claude Desktop、IDE 插件等）都能即插即用。截至 2026 年初，已有数千个社区贡献的 MCP Server 可用，覆盖数据库、API、文件系统等常见场景，生态仍在快速扩展中
...
- **[[理论学习_ReAct_范式与_Agent_Loop|ReAct]] 模式** ：最经典的 Agent 工作范式——Reasoning（推理）和 Acting（行动）交替进行。模型先想我应该做什么，然后执行一个动作（比如调用搜索工具），观察返回结果，再决定下一步。大多数 Agent 框架都基于这个模式
...
> 当然，这里只是列了核心链路，像问题重写、会话记忆、[[01基础_18意图识别与多路由调度策略|意图识别]]、MCP、安全护栏等都没有讲，只是让大家有个概念。
...
没有记忆的 AI 每次对话都是失忆状态——用户说了上半句，下半句它就忘了。不管是 RAG 问答还是 Agent 交互，只要涉及[[01基础_16多轮对话记忆设计|多轮对话]]，Memory 都是必须的。
