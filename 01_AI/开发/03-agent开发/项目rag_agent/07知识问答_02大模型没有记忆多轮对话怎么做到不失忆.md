...
看起来很轻，但这一行背后藏着[[理论学习_跨会话记忆与会话持久化|记忆系统]]的完整设计——存储、加载、并行拉取、降级容错、摘要注入，全都压缩在这一个方法调用里。
...
| 位置 | 角色 | 内容 | 设计考量 |
| --- | --- | --- | --- |
| 最前 | SYSTEM | [[06-RAG|RAG]] 角色规则 | 优先级最高的行为约束，大模型会把最前面的 SYSTEM 消息当作最高优先级指令 |
...
- 4.
	**注入顺序** ：系统 Prompt → [[理论学习_MCP_协议与开放工具生态|MCP]] 数据 → KB 检索结果 → 摘要（SYSTEM）→ 原文历史 → 当前问题。每个位置都有设计考量，最终让大模型在充分的上下文中回答用户的最后一个问题。
