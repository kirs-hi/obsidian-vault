...
不管你现在做的是业务系统还是中间件，面试的时候多多少少都会被问到 AI 相关的东西。[[06-RAG|RAG]] 是什么？[[07-Agent|Agent]] 怎么实现？用过 [[理论学习_MCP_协议与开放工具生态|MCP]] 吗？这些问题越来越高频。可以说， **AI 已经从“加分项”变成了“必答题”** 。
...
这个转换由专门的 [[01基础_08从文本到向量之理解Embedding|Embedding]] 模型完成，比如 Qwen3 的 Qwen3-Embedding-8B，或者开源的 bge、m3e。

#### 2.4 Index：存进向量数据库

向量算出来了，得找个地方存起来。

普通数据库存结构化数据，做精确匹配。[[01基础_09向量数据库的原理与选型|向量数据库]]专门存向量，做相似度搜索——给一个向量，找出距离最近的 N 个。
...
|  | [[01基础_06数据分块Chunk策略与实践|Chunk]] | 切成小块 |
|  | Embed | 转成向量 |
|  | Index | 存进向量数据库 |
...
实际落地的时候，每一步都有不少讲究。比如文档怎么切块效果最好？向量模型选哪个？检索结果要不要做[[06调度引擎_08Rerank重排序与辅助工具|重排序]]？这些选择没有标准答案，得看你的数据特点和业务场景。
...
> 观察到一个变化：阿里云 AI 助理的流程/[[提示词]]可能做了收敛。此前在带前置描述的情况下，模型会输出明显的拟人化/角色扮演内容（例如“尊敬的奥特曼先生，感谢您为光之国做的贡献……”）。这类输出虽有趣，但对 ToC 场景会带来风格漂移与严谨性不足的问题。我认为这是一次合理的优化。
...
- **口语化** ：用户问“报销咋整”，你拿这四个字去做[[01基础_10向量检索策略与召回优化|向量检索]]，效果能好吗？得把它展开成“公司报销流程是什么，需要准备哪些材料”之类的完整表述，检索效果才会好。

- **[[01基础_16多轮对话记忆设计|多轮对话]]** ：用户第一轮问“年假有多少天”，第二轮接着问“怎么申请”。这个怎么申请单独拿去检索，系统根本不知道在问啥。得把上文的年假补进去，变成年假怎么申请，才能检索到正确的内容。
...
[[01基础_18意图识别与多路由调度策略|意图识别]]做得不好，后面的流程再完美也没用——方向错了，跑得越快错得越远。
