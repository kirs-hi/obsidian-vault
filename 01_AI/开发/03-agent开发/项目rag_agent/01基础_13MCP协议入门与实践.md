...
上一篇讲了 [[01基础_12理解函数调用Function Call|Function Call]]，你已经能让 [[06-RAG|RAG]] 系统从只能查知识库升级到能查数据、能调接口、能干活。模型自己判断什么时候该调工具，输出标准化的调用意图，你的代码执行函数——整个流程跑得很顺。
...
- **[[01基础_16多轮对话记忆设计|多轮对话]]机制成熟** ：定义工具 → 模型输出调用意图 → 执行函数 → 返回结果 → 生成答案，流程清晰
...
- 本地进程（通过 Stdio 通信，比如一个本地的[[08-文件操作|文件操作]]工具）
...
MCP 协议定义了三大核心能力：Tools（工具调用）、Resources（资源访问）、Prompts（[[提示词]]模板）。
...
**[[理论学习_MCP_协议与开放工具生态|MCP协议]]层面** （标准制定者）：
...
- 支持[[01基础_20SSE协议与流式响应|流式响应]]（SSE），适合长时间运行的工具
...
| [[Claude Code 命令与最佳实践|Claude Code]] | CLI 工具 | 完整支持 |
...
到这里，RAG 系列的核心环节已经讲完了：[[01基础_06数据分块Chunk策略与实践|数据分块]] → 元数据管理 → 向量化 → [[01基础_09向量数据库的原理与选型|向量数据库]] → 检索策略 → 生成策略 → Function Call → MCP 协议。从数据准备到检索生成，再到工具调用和协议标准化，一条完整的链路。
