...
上一篇讲了 runner 的双接口聚合——一条 query 跑两个接口，[[01基础_20SSE协议与流式响应|SSE]] 拿答案和 TTFT，JSON 拿检索证据和意图分类，合并成 `EvalRecord` 落进 `runs/*.jsonl` 。20 条样本跑完，20 行 JSON 静静躺在那里。
...
[[06-RAG|RAG]] 系统的处理链路是一条单向管道：[[01基础_18意图识别与多路由调度策略|意图识别]] → 路由 → 检索 → 生成。意图是管道的第一个阀门，错了之后水流方向就偏了，后面每一步都在错误的方向上努力。
...
> 将来 Ragent 升级到 v2 版本后，可能会出现仅通过 [[07-Agent|Agent]] 循环获取答案，不走 RAG，所以 requires\_rag 也会等于 false。
...
这些指标全部是纯集合运算，不调 LLM，秒级出结果。每次改了意图描述、换了 [[01基础_08从文本到向量之理解Embedding|Embedding]] 模型、调了 rerank 参数，跑一遍就知道效果是变好还是变差——这正是自建指标存在的意义。
