...
上一篇把[[01基础_18意图识别与多路由调度策略|意图识别]]和问题路由讲完之后，[[06-RAG|RAG]] 系统的各个环节已经全部串起来了：数据入库 → 向量化 → 检索 → 生成 → 工具调用 → 会话记忆 → Query 改写 → 意图识别。系统跑起来了，用户在用了，看着也挺好的。

但有个问题一直没回答—— **这个系统到底好不好？**

你上周把 chunk size 从 512 改成了 1024，改完之后抽了 10 个问题问了一下，感觉回答质量还行。但还行到底有多好？有没有其他问题因为这次修改变差了？你不知道。

你前天换了一个新的 [[01基础_08从文本到向量之理解Embedding|Embedding]] 模型，换完之后试了几个问题，检索结果看着不错。但看着不错和真的不错之间差着一个评估体系。
...
- Hit Rate 80%，低于 85% → 检索阶段有问题，退货运费谁承担没有命中正确 chunk。分析原因：可能是退货运费和 chunk\_13 的文本退货运费由买家承担语义匹配度不够，或者 Top-3 太少了。优化思路：把 Top-K 从 3 调到 5，或者加入[[06调度引擎_08Rerank重排序与辅助工具|重排序]]。

- 忠实度 3.33，整体偏低，退货运费忠实度 1 分、跨境商品退货和 Apple Watch 防水各 3 分 → 生成阶段的 Prompt 需要加强限定。优化思路：在 [[理论学习_System_Prompt_如何设计_|System Prompt]] 里加一条严禁添加参考文档中未出现的信息，对未检索到内容的场景强化兜底指令。
...
**数据入库 → 分块 → 元数据 → 向量化 → [[01基础_09向量数据库的原理与选型|向量数据库]] → 检索策略 → 生成策略 → [[01基础_12理解函数调用Function Call|Function Call]] → [[理论学习_MCP_协议与开放工具生态|MCP]] 协议 → 会话记忆 → Query 改写 → 意图识别与路由 → 评估与优化**
