...
上一篇讲完了文档的生命周期管理——删除、更新、启用/禁用，都是针对文档整体的操作。但文档粒度太粗了，自动分块之后，实际参与检索的是 [[01基础_06数据分块Chunk策略与实践|Chunk]]。用户有时候需要针对某一段具体内容做调整，文档级操作够不到这个层面。
...
这个字段目前的用途是 **管理后台的展示** ——让知识库管理员在 Chunk 列表里直观看到每个分块的大致 Token 规模，方便判断哪些 Chunk 太长或太短。它不参与 [[06-RAG|RAG]] 检索链路的任何逻辑计算。
...
整个 `create` 方法加了 `@Transactional` ，但 `syncChunkToVector` （调用 [[01基础_08从文本到向量之理解Embedding|Embedding]] API + 写向量库）是在事务内执行的。这里和 batchEnable 的做法不同：单条 Chunk 的向量化耗时短（一次 API 调用几百毫秒），放在事务内还可以接受；批量操作时 embed 多条，总耗时可能很长，那时候才需要把 embed 移到事务外。
...
`vectorStoreService.updateChunk` 这个方法名叫 update，但[[01基础_09向量数据库的原理与选型|向量数据库]]底层的实现通常是删旧插新。向量数据库中存的是 `(id, vector, payload)` ，虽然 chunkId 没变，但对应的 vector 完全不同了（内容变了，向量就变了），更新语义是替换向量数据，而不是修改某个值。
