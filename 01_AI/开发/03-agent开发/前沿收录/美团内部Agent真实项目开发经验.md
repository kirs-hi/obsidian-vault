...
> 整理目的：数仓开发转行 [[07-Agent|Agent]] 开发，收集**有完整开发链路**（背景痛点 → 技术选型 → 架构 → 里程碑 → 踩坑 → 落地效果）的内部实战文档，可用于简历素材与面试讲述。
...
| 范式 | 核心思路 | 适用场景 | 代表实现 |
| --- | --- | --- | --- |
| [[理论学习_ReAct_范式与_Agent_Loop|ReAct]] | 交替 Reasoning 和 Acting，每步观察环境更新状态 | 需动态获取信息的单 Agent 任务 | LangChain ReAct Agent |
...
| Agentic [[06-RAG|RAG]] | Agent 驱动的多轮迭代检索 + 推理 | 复杂知识问答、研究报告 | LlamaIndex Agentic RAG |

三大协议（高频考点）：

- **[[理论学习_MCP_协议与开放工具生态|MCP]]**（Anthropic 2024.11）：Agent ←→ 工具/资源，JSON-RPC 2.0，Tools/Resources/Prompts 三原语
- **A2A**（Google 2025.4）：Agent ←→ Agent，HTTP/[[01基础_20SSE协议与流式响应|SSE]]，Agent Card 发现
...
长任务可靠性方案：Checkpointing（状态持久化断点续跑）、Plan 重规划（Reflexion）、Human-in-the-loop（关键节点人工确认）、[[理论学习_上下文压缩与_Token_管理|上下文压缩]]（LLMLingua）、失败预算（最大重试 + 超时）。

C2 面试高频题（自测清单）：

1. 解释 ReAct，与 Chain-of-Thought 的区别？
2. MCP 协议解决了什么问题？与 [[01基础_12理解函数调用Function Call|Function Calling]] 区别？
