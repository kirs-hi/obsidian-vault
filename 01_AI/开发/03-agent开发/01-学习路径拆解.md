...
A curated AI [[07-Agent|Agent]] learning roadmap for people who want to build useful, reliable agents instead of collecting random links.
...
| Priority | Learn | Why |
| --- | --- | --- |
| 1 | [[Claude Code 命令与最佳实践|Claude Code]] / Codex-style coding agents | 真实代码库、shell、文件编辑、测试、权限、[[理论学习_上下文压缩与_Token_管理|上下文压缩]]，是最好的 agent 工程样本。 |
| 2 | Agent harness engineering | agent 的能力很大一部分来自 harness：工具协议、权限、状态、反馈、回放、CI、评测。 |
| 3 | [[00_OpenClaw_MOC|OpenClaw]] / Hermes-style personal agents | 长运行、本地优先、跨应用、记忆、skills、消息入口，更像“个人操作系统”。 |
| 4 | Skills / [[理论学习_MCP_协议与开放工具生态|MCP]] / A2A / ACP | skills 负责能力复用，MCP 连接工具，A2A 连接 agent，ACP 连接宿主应用。 |
...
| [Onyx](https://github.com/onyx-dot-app/onyx) | 企业级 [[06-RAG|RAG]]/search assistant，适合学 connectors、hybrid search、权限和生产化。 |
...
| Tutorial Encyclopedias | [GenAI_Agents](https://github.com/NirDiamant/GenAI_Agents), [hello-agents](https://github.com/datawhalechina/hello-agents), [smolagents](https://github.com/huggingface/smolagents), [agents-towards-production](https://github.com/NirDiamant/agents-towards-production) | 横向看 [[理论学习_ReAct_范式与_Agent_Loop|ReAct]]、Plan-and-Execute、Multi-Agent、production patterns。 |
...
| [pi](https://github.com/earendil-works/pi) | AI agent toolkit（[[Day2-JavaScript和TypeScript|TypeScript]]），含 coding agent CLI、agent core、统一多提供商 LLM API 和 TUI 库，支持自扩展。 |
