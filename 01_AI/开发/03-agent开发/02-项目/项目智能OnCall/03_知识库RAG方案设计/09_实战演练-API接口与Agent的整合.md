...
1. 代码路径： `SuperBizAgent/internal/controller/chat/chat_v1_file_upload.go`
2. 首先将用户上传到文件保存到本地
3. `buildIntoIndex` 然后去数据库中查找是否有元数据一样的文件，如果一样，则说明用户是更新文档，则先删除原来的数据。
4. 调用 `knowledge_index_pipeline.BuildKnowledgeIndexing(ctx)` 创建知识库Agent的执行器
5. 调用 `r.Invoke` 执行Agent，将文件向量化。（ `BuildKnowledgeIndexing` 在《RAG代码实战1》里有详细解释）
6. 最后构造返回结构体返回请求
...
2. 将文件内容保存到本地 uploads/ 目录，如果同名文件已存在则先删除（覆盖更新）
3. 调用 vector\_index\_service.index\_single\_file 对文件进行向量化索引存储
4. 构造响应结构体返回请求
