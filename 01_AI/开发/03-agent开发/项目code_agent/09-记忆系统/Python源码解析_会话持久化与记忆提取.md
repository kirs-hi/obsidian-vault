...
一条用户消息对应一条记录，但一条带工具调用的助手回复会把文本和工具调用打包成一个 content\_ blocks 列表。而工具结果那边，每个工具结果单独一条记录。这种不对称的设计有原因：Anthropic API 要求同一个 assistant 消息里的文本和工具调用必须打包在一起，但工具结果可以分开。
...
### 读前校验（read-before-edit）

直接在工具层卡住「没读就改」的问题。 `FileStateCache` 记录每个文件被 ReadFile 读取时的内容和修改时间，EditFile / WriteFile 执行前做两道检查：
...
两道门：第一，文件有没有被 ReadFile 读过？没有就拒绝。第二，文件自从上次读取后有没有被外部改过（比如别人 git pull 了新代码）？改过也拒绝，必须重新读取。
...
| 读前校验 | FileStateCache 记录 mtime，EditFile/WriteFile 执行前双重检查 |
