...
上一章我们给 MewCode 装上了跨会话[[理论学习_跨会话记忆与会话持久化|记忆系统]]，它终于能记住你的偏好和项目上下文了。但有些日常操作还是让人哭笑不得——你想清个屏，[[07-Agent|Agent]] 认认真真地回复「好的，已帮你清除」，实际上什么也没发生，还白白消耗了 token。

这一章要给 MewCode 装上 Slash Command 内置命令框架。做完之后，所有以 `/` 开头的常用操作都会绕过 [[理论学习_ReAct_范式与_Agent_Loop|Agent Loop]] 直接在本地执行，省 Token、响应快、行为确定。
...
这是[[提示词]]命令，执行的时候会直接把我们的这段提示词发给模型，也就是相当于复用prompt，执行起来的效果就如下
