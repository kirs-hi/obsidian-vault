理论篇讲了 [[理论学习_System_Prompt_如何设计_|System Prompt]] 的七个信息来源和三条通道，这篇带你走读 Go 版 MewCode 的真实代码，看看 Prompt 组装管线是怎么一步步拼出最终发给 LLM 的那段长文本的。
...
| `plan_mode.go` | Plan Mode [[提示词]]模板（完整版 / 精简版 / 退出版），BuildPlanModeReminder 控制注入频率 |
...
这三个字段对应「条件注入」的部分。有值就加，没值就跳过。后续章节的 Skill 系统、[[理论学习_跨会话记忆与会话持久化|记忆系统]]就是通过这个入口把自己的内容塞进 System Prompt 的。
...
| `UsingToolsSection` | 40 | 工具使用指南：专用工具优先于 Bash、并行调用、[[07-Agent|Agent]] 委派 |
...
Plan Mode 的提示词不走 `BuildSystemPrompt` ，而是在 [[理论学习_ReAct_范式与_Agent_Loop|Agent Loop]] 里通过 `system-reminder` 通道单独注入。它有三个模板：
