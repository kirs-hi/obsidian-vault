...
上一章我们把 [[理论学习_ReAct_范式与_Agent_Loop|Agent Loop]] 跑起来了。
...
这一章要把这三行展开成一套完整的 Prompt 工程体系。做完之后，MewCode 的 [[理论学习_System_Prompt_如何设计_|System Prompt]] 会有清晰的七模块结构，组装管线会按七源到三通道的规则正确分发信息，Prompt Cache 会真正命中并把每轮的 input token 成本打下来。Plan Mode 的指令也从硬拼接改成走 system-reminder 通道，不再每次都让缓存失效。
...
这章 **不做** ：MEWCODE.md 项目指令文件加载（章节 7）、自动[[理论学习_跨会话记忆与会话持久化|记忆系统]]（章节 9）、真实 [[理论学习_MCP_协议与开放工具生态|MCP]] Server 接入（章节 6）、LLM-as-judge 自动评估管线（依然是人工跑场景对比）。
