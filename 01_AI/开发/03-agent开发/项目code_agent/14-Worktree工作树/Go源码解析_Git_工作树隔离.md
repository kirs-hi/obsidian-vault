理论篇讲了为什么并行 [[07-Agent|Agent]] 需要文件系统隔离，以及 Git Worktree 的机制。这篇带你走读 Go 版 MewCode 的 `internal/worktree/` 目录，看看 10 个文件、约 1400 行代码是怎么实现完整的 Worktree 生命周期管理的。
...
| `notice.go` | 19 | 注入子 Agent [[提示词]]的路径翻译通知 |
