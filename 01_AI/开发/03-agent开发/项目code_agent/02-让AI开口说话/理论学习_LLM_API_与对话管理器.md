**架构定位** ：本章横跨两层。② 引擎层的 LLM 客户端和对话管理器是 [[07-Agent|Agent]] 的基础设施，后面的 Agent 循环和子 Agent 都建立在它之上。① 交互层部分，章末简要介绍终端 UI 的目标形态。
...
这一章开始动手。做两件事： **调通 LLM** **API****，然后套上终端界面支持[[01基础_16多轮对话记忆设计|多轮对话]]** 。

API 调用看着简单，但里面藏着 Agent 架构的核心概念。消息格式决定了对话怎么组织，[[01基础_20SSE协议与流式响应|流式响应]]决定了用户体验的下限，Token 计费决定了你的钱包能撑多久。
...
[https://claude.com/app-unavailable-in-region](https://claude.com/app-unavailable-in-region)
...
**tools 参数** 放的是工具描述。你的 Agent 有哪些工具可以用、每个工具的参数是什么格式、返回什么结果。这就是 [[01基础_12理解函数调用Function Call|Function Calling]]，工具系统章节会详细展开。
...
![](理论学习_LLM_API_与对话管理器-8.jpeg)

但更隐蔽的成本陷阱在别的地方。

![](理论学习_LLM_API_与对话管理器-9.jpeg)
...
这就是为什么后面要专门讲[[理论学习_上下文压缩与_Token_管理|上下文压缩]]。目前我们先把 Token 用量显示出来，让自己对成本有个直观感知。
...
但对于一个 [[理论学习_什么是_Coding_Agent_|Coding Agent]] 来说，多轮对话是基本能力。用户描述一个需求，Agent 问几个澄清问题，然后开始执行。这个过程天然就是多轮的。
...
![](理论学习_LLM_API_与对话管理器-17.jpeg)

其实也就是参考ClaudeCode这种简洁UI来实现

![](理论学习_LLM_API_与对话管理器-18.jpeg)
