...
上一章我们给 MewCode 接上了 [[理论学习_MCP_协议与开放工具生态|MCP]] Client，它终于能通过标准化协议动态接入外部工具了。但随着 [[07-Agent|Agent]] 能力越来越强，一个新问题浮出水面：它干活的时间越来越长，读的文件越来越多，Token 消耗直线飙升，直到撞上上下文窗口的天花板。

这一章要给 MewCode 装上[[理论学习_上下文压缩与_Token_管理|上下文管理]]能力。做完之后，Agent 能在有限的 Token 预算内长时间工作，不会因为对话太长而瘫掉。
...
-   **[[理论学习_ReAct_范式与_Agent_Loop|Agent Loop]] 集成** ：每轮循环前自动执行两层压缩，压缩结果赋值回外层对话历史
...
验收没问题，那么本章的主要任务就完成了。事实上这个只是Agent的[[理论学习_跨会话记忆与会话持久化|记忆系统]]的一部分，下一章，我们来进一步巩固我们的记忆系统更完整的体系
