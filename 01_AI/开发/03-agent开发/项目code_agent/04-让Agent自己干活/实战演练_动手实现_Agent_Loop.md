...
上一章我们给 MewCode 装了六个工具，实现了 Function Calling，它能读文件、写文件、搜代码、执行命令。但每次只能做一步——模型返回一个 tool\_ use，你执行完返回结果，模型给个最终回复，结束。你得一步步催它。
...
⬅️ 上一篇：[[TypeScript源码解析_Agent主循环与事件流]]
