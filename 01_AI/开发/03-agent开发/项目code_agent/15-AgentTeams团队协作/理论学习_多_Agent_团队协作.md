**架构定位** ：本章扩展 引擎层，在 [[理论学习_SubAgent_子任务分发|SubAgent]] 基础上引入多 [[07-Agent|Agent]] 协作机制。多个 Agent 通过共享任务和点对点消息实现自组织协作。
...
Agent Team 接近「网状」结构，每个队员有自己的上下文窗口，可以直接给其他队员发消息，不必绕回 Lead 中转。不过这里要先说清楚一个细节，所谓「直接发消息」并不是网络层面的实时直连，底层走的是共享文件邮箱加 500ms 轮询，消息会在收件人下一轮 [[理论学习_ReAct_范式与_Agent_Loop|Agent Loop]] 开头被读到。换句话说，队员之间可以直接寻址，但通信本质上是异步的。共享任务列表加这套异步邮箱，就是 Team 的协调底座。

![](理论学习_多_Agent_团队协作-3.jpeg)
...
![](理论学习_多_Agent_团队协作-24.jpeg)

### 四阶段工作流

Coordinator Mode 不只是收窄工具集，系统还会注入一套 coordinator 系统[[提示词]]，把大部分任务分解成四个阶段：
