...
3. **智能体能力的落地**
   ...
   * 其它模型也在尝试 [[07-Agent|Agent]]，但 Gemini 3 搭配 **Antigravity 平台**，实现了端到端工作流：不仅能调用工具，还能 **自主规划、执行代码、验证结果**，真正从“回答问题”进化到“完成任务”。
...
这一章，首先讲解 7.1 讲解市面上所有的主流模型的典型特点。 7.2 结合[[面试题]]：你的项目使用什么模型 来讲解 项目模型选择的策略。 7.3 讲解一下最新典型旗舰模型的特点。
...
\- **How（工作流程）**：用户发起对话 → 意图分类 → 分流至预约 / 咨询机器人 → 数据库查询 / [[06-RAG|RAG]] 检索 → 确认预约并推送天气提醒。
...
\- **工具调用 / [[01基础_12理解函数调用Function Call|Function Call]]（必要，30%）**：预约机器人需要精准调用数据库查询技师空档。

\- **RAG 检索整合（必要，20%）**：咨询机器人须从私有文档中准确检索按摩房信息。

\- **[[01基础_16多轮对话记忆设计|多轮对话]]上下文理解（必要，10%）**：预约流程需多轮收集用户偏好，不能"失忆"。

\- **[[理论学习_MCP_协议与开放工具生态|MCP]] 外部服务集成（加分，5%）**：预约成功后调用天气 MCP Server，生成个性化提醒。
