...
起初我以为是 Prompt Engineering 的问题，尝试了多次优化 [[理论学习_System_Prompt_如何设计_|System Prompt]]，但无效。于是我决定下沉到**协议交互层**进行排查：
...
在这个模块化 [[06-RAG|RAG]] 项目中，我负责实现 MCP Server 模块，通过 [[理论学习_MCP_协议与开放工具生态|MCP协议]]将 RAG 检索能力暴露给 AI 客户端（如 Claude Desktop、GitHub Copilot）。我们的传输层采用的是 stdio 模式——Server 作为子进程启动，通过 stdin/stdout 与客户端进行 JSON-RPC 通信。
