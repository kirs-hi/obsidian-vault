...
- 17.4 技术细节
  ...
  - 17.4.2 任务型对话
  - 17.4.3 信息型对话
  - 17.4.4 生成"命令生成模型"的微调数据
  - 17.4.5 命令生成模型微调与部署
  - 17.4.6 信息型对话（GraphRAG）
- 17.5 项目总结
- 17.6 疑难解析
  - 17.6.1 其他可用框架，Rasa Pro的选用原因。
  - 17.6.2 客服系统常见业务评价指标
  - 17.6.3 对话管理怎么做的
  - 17.6.4 意图识别怎么做的，如何接入大模型
  - 17.6.5 意图的数量，评价指标有哪些
  - 17.6.6 用户描述清洗的情况下，依然无法匹配正确工作流
  - 17.6.7 Graph RAG的使用原因，使用方式
  - 17.6.8 对话模型为何使用Qwen
  - 17.6.9 客服系统的FAQ
  - 17.6.10 对话模型为何是8B
  - 17.6.11 对话任务执行的流程
  - 17.6.12 执行RAG工作流时，如何平衡检索速度与答案准确性
  - 17.6.13 模型训练的数据来源，如何构建
  - 17.6.14 优化模型推理速度的方法有哪些？
...
选用Rasa Pro，是出于工程性、稳定性的考虑。Rasa Pro框架提供强大的对话管理工具、支持多语言意图识别/实体识别能力、具有**可视化对话管理工具rasa studio、支持非线性对话、私有化部署及本地部署模型调用**，并且框架整体可控性较好（可自行调优意图识别、调优对话策略），发展（版本更新）也较为稳定。
...
意图识别本质上是一个**多分类任务，**Rasa Pro会在config.yml中定义一个NLU PipeLine，通常会使用到Rasa Pro提供的DIETClassifier模型（百万参数的小模型），模型可以同时完成意图分类与实体识别。
