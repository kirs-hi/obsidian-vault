---
title: "【中等项目 1 实战-04】从 Stitch 生成小程序界面"
source: "https://articles.zsxq.com/id_2lnwrqarzkxb.html"
author:
  - "[[AI随风]]"
published:
created: 2026-05-23
description:
tags:
  - "clippings"
---
[来自： AI随风的AI编程实战营](https://wx.zsxq.com/group/51115228418814)

## 小程序的界面结构

## 页面组成

小程序页面是由四个文件组成，缺一不可

![](https://article-images.zsxq.com/Fu354EGCYEDc7A1slOKfmurzGFyB)

在早期的时候，AI生成页面的时候有可能会漏缺文件，所以可以在配置里面配置一下规则

![](https://article-images.zsxq.com/FtgN5ycOwRWIi3DgPd0TXoJS3jIY)

这种rules意思就是当claude修改这个文件目录的文件时，会触发这个规则。这种规则不属于全局规则，所以我们用rules来进行分模块，在多个模块的项目中，这是一种非常常用的手段

## Tabbar

Tabbar是小程序的底部导航，这个跟别的APP页面非常的不一样的地方，所以在处理的时候要特别注意，每一个底部的都要有图片表示菜单选中和非选中状态

![](https://article-images.zsxq.com/FqrXSlWHHGlwhwtTFfodS_mEhTKy)

## 顶部导航

因为小程序的顶部导航除了自定义，都是在page.json里面配置 的，如果不特别说明，这里非常容易出错，模型生成的代码经常会把页面中原来的导航带进来

## stitch 生成小程序的方法

我们要从外部源头来生成小程序界面，这个外部源头有两种方式：

1、MCP 获取

2、html 源文件或者截图

其实MCP的底层就是去下载HTML文件。我们用这两种方式操作都可以。

## 配置MCP

打开 stitch 某个项目，选择导出

![](https://article-images.zsxq.com/Fk9TeGHz-mX3IiarkEWJeFHeTzz-)

选择 MCP，选择 Cladue Code, 复制配置，发送给 claude Code安装就可以了

![](https://article-images.zsxq.com/FhTks8M83rMBM-QJ3oxoksIAQQDp)

## 下载html文件

![](https://article-images.zsxq.com/Fny_T8yPgn7PtqxzDENhMZY7Vtj_)

选中某个页面，点导出，选择zip格式下载下来，你将会得到一个code.html 这个可以认为叫做源文件

![](https://article-images.zsxq.com/FsG61h7-cXG0pJU8_FpRKlhVNFk5)

## 生成小程序页面

为了处理HTM转成小程序，我特别写了 [个技能【在另一个目录进行下载】](https://t.zsxq.com/oKVhO) ，这个技能会将html里面的元素和小程序的元素进行 1:1 转换，特别处理 tabbar和顶部的navigate 导航。

1. 使用 MCP 进行转换

点击某个页面，点击复制，然后输入以下提示词

![](https://article-images.zsxq.com/FknE0jOrKg13OC3Pw69e3WQcbCtw)

/html-to-uniapp-page 使用 stitch mcp 将 https://stitch.withgoogle.com/projects/4523681392850147107?node-id=197d2254bc224770a5492a38504f0b36 页面转换为小程序页面， 小程序页面名字是index

1. 使用 html 转换

/html-to-uniapp-page 将 code.html 页面转换为小程序页面， 小程序页面名字是index

最终效果，然后继续调整页面，继续转换页面

![](https://article-images.zsxq.com/FjtSAK0UFxWiAqXKP2u4IgVwyj4p)

![](data:image/png;base64,)

扫码加入星球

查看更多优质内容

https://wx.zsxq.com/mweb/views/joingroup/join\_group.html?group\_id=51115228418814

<!-- series-nav-start -->

---
**📚 中等项目 1 实战**（4/9）

⬅️ 上一篇：[[【中等项目 1 实战-03】小程序页面制作，Stitch使用指南]] | ➡️ 下一篇：[[【中等项目 1 实战-05】生成后台管理界面及表结构]]

<!-- series-nav-end -->
