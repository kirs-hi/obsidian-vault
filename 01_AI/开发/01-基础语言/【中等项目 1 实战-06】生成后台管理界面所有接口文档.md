---
title: "【中等项目 1 实战-06】生成后台管理界面所有接口文档"
source: "https://articles.zsxq.com/id_28gh3eqqwn83.html"
author:
  - "[[AI随风]]"
published:
created: 2026-05-23
description:
tags:
  - "clippings"
---
[来自： AI随风的AI编程实战营](https://wx.zsxq.com/group/51115228418814)

## 前置任务

在上一个任务中，已经完成了后台管理系统所有界面的开发，需要花一点时间把所有页面的交互都调试正确，字段显示正确，而且所有的数据必须是来自于mock数据，也就是说不是写在页面上的，是定义的数据结构

![](https://article-images.zsxq.com/FgqtIjULJyXduP8WPwu7oeNECsjh)

只要按照上一章的方法，那么你就可以做成这样子

## 该直接生成代码还是接口文档？

在这一步，其实有两种方法：

**1、指定后端的某些模块，然后直接让它生成后端的代码**

/pan 或者任何 openspec: 请查看frontend项目关于房源模块的所有 mock 数据，完成相应的后端逻辑代码编写

这种方法适合于项目可能比较简单，后端逻辑模块比较少,因为会带来的问题是，AI在完成这个模块的时候完成的挺好的，但是在完成下一个模块的时候，可能会忘记前一个模块定义好的一些结构、公共的函数等等。

而且你在开始每一个模块的时候，都有可能是会被AI认为是从零开始，你的接口参数的输入输出可能会被定义多种形式。你可能无法审阅它生成的具体的互动逻辑

**2、根据模块，生成模块的所有接口文档**

这种方法相当于是把所有的模块都扫一遍，然后生成每个模块对应的接口文档，那这个接口文档就包含了接口的定义、公共函数的引入以及复杂接口的逻辑等等，那么你就可以去审阅每一个接口的输入输出以及重要接口的流程是否有问题，这给我们下一步生成后端代码提供了一个非常好的上下文内容

## 接口文档的定义

下面我演示讲解一下我生成的一个接口文档到底包含了什么内容

这是所有生成模块接口文档，放在 docs/api 中

![](https://article-images.zsxq.com/FlczukgUUNoCM3yprScxnz5PAVNJ)

![](https://article-images.zsxq.com/FhLy1KBDxCrNsFzo2PYc4t1QLIGO)

1. Api-common.md 是接口的统一规范，包含了输入输出的标准，分页增删改查的定义，以及怎么登录标识错误码等

\# API 公共规范

\## 1. 统一响应结构

所有接口统一返回以下 JSON 结构：

\`\`\`json

{

"code": 200,

"message": "操作成功",

"data": {}

}

\`\`\`

| 字段 | 类型 | 说明 |

| --- | --- | --- |

| code | Integer | 业务状态码，200 表示成功 |

| message | String | 提示信息 |

| data | Object \\| Array \\| null | 业务数据，无数据时为 null |

\### 成功响应示例

\`\`\`json

{ "code": 200, "message": "操作成功", "data": { "id": 1, "name": "集团管理员" } }

\`\`\`

\### 失败响应示例

\`\`\`json

{ "code": 400, "message": "角色名称不能为空", "data": null }

\`\`\`

\### 无数据返回

\`\`\`json

{ "code": 200, "message": "操作成功", "data": null }

\`\`\`

\## 2. 分页查询

\### 请求参数

| 参数 | 类型 | 默认值 | 说明 |

| --- | --- | --- | --- |

| page | Integer | 1 | 当前页码，从 1 开始 |

| pageSize | Integer | 20 | 每页条数，最大 100 |

| sortBy | String | - | 排序字段（驼峰命名） |

| sortOrder | String | desc | 排序方向：asc / desc |

\### 响应结构

\`\`\`json

{

"code": 200,

"message": "操作成功",

"data": {

"list": \[\],

"total": 100,

"page": 1,

"pageSize": 20

}

}

\`\`\`

| 字段 | 类型 | 说明 |

| --- | --- | --- |

| list | Array | 当前页数据列表 |

| total | Integer | 总记录数 |

| page | Integer | 当前页码 |

| pageSize | Integer | 每页条数 |

\## 3. 错误码规范

\### 通用错误码

| 错误码 | 说明 |

| --- | --- |

| 200 | 成功 |

| 400 | 请求参数错误 |

| 401 | 未认证（未登录或 token 过期） |

| 403 | 无权限 |

| 404 | 资源不存在 |

| 409 | 业务冲突（如重复操作、状态不允许） |

| 500 | 服务器内部错误 |

\### 业务错误码

| 区间 | 模块 |

| --- | --- |

| 10001-10999 | 用户与权限 |

| 20001-20999 | 房源管理 |

| 30001-30999 | 求租信息 |

| 40001-40999 | 订单与支付 |

| 50001-50999 | 收益与分账 |

| 60001-60999 | 客户管理 |

| 70001-70999 | 合同管理 |

| 80001-80999 | 投诉监管 |

\## 4. 认证方式

所有需要认证的接口，请求头携带：

\`\`\`

Authorization: Bearer <token>

\`\`\`

token 有效期由后端配置，过期返回 401。

\## 5. 通用字段约定

| 字段 | 类型 | 格式 | 说明 |

| --- | --- | --- | --- |

| id | BIGINT UNSIGNED | 自增 | 主键 |

| createdAt | DATETIME | yyyy-MM-dd HH:mm:ss | 创建时间，自动填充 |

| updatedAt | DATETIME | yyyy-MM-dd HH:mm:ss | 更新时间，自动更新 |

| deletedAt | DATETIME \\| null | yyyy-MM-dd HH:mm:ss | 软删除时间，null 表示未删除 |

| status | TINYINT UNSIGNED | 枚举值 | 各表含义不同，见模块文档 |

\## 6. 金额字段

统一以\*\*分\*\*为单位（Integer），前端展示时除以 100。

示例：租金 50000 分 = 500.00 元。

1. 涉及页面

是所有这次修改可能会涉及的前端页面，这样我们在下一步生成代码的时候可以把前端的对接也做好

1. 涉及到表结构

![](https://article-images.zsxq.com/FtKPLiCmlJt7WOdLsqQwAooAvXRx)

1. 接口总览

![](https://article-images.zsxq.com/FjmmWUAiIz8fuBCLA1hUB-JuWqmT)

1. 接口详情

![](https://article-images.zsxq.com/Flr1Ke9_9vuxwdYWXpStOZAF-W_0)

你会列出每一个接口的输入输出参数以及示例参数

1. 重要流程

![](https://article-images.zsxq.com/FldV0ovl8AxRAgWVh_kbfE3f0et-)

有了上面这些基础的定义，那生成后端代码就基本上没什么问题。需要认真审阅这个接口文档。

## 生成接口文档

[我专门写了一个技能来生成接口文档，因为其实所有的流程都非常通用](https://t.zsxq.com/7m3H7)

/api-doc-infer 请收集 front-end 项目关于房源模块的接口文档

![](data:image/png;base64,)

扫码加入星球

查看更多优质内容

https://wx.zsxq.com/mweb/views/joingroup/join\_group.html?group\_id=51115228418814

<!-- series-nav-start -->

---
**📚 中等项目 1 实战**（6/9）

⬅️ 上一篇：[[【中等项目 1 实战-05】生成后台管理界面及表结构]] | ➡️ 下一篇：[[【中等项目 1 实战-07】使用 TDD 方式生成和测试代码]]

<!-- series-nav-end -->
