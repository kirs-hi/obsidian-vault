---
title: "【Claude Code-06】创建自定义命令"
source: "https://articles.zsxq.com/id_r7vzhw347b2s.html"
author:
  - "[[AI随风]]"
published:
created: 2026-05-23
...
注意：但凡你的 [[提示词]]使用超过两次，都应该封装成命令或者技能

## Slash Commands 作用范围

在看过这么多的系统命令之后，是否也想过创建自己的命令呢？那创建命令也是非常简单的，

命令的作用范围有两个，一个是全局的，一个是项目目录
...
$ARGUMENTS： 代表命令后面的所有参数，

$0，$1 代表第一个，第二个参数

![](https://article-images.zsxq.com/FmWqYt8MClQlDwuebE90hR2B-hbC)
...
审查最近修改的代码，识别并消除不必要的复杂性，在不改变行为的前提下提升可读性和可维护性。

\## 执行步骤

1\. \*\*识别目标\*\*：检查 \`$ARGUMENTS\` 指定的文件，若未指定则检查最近修改的文件（\`git diff --name-only HEAD\`）
...
3\. \*\*执行简化\*\*：逐文件修改，每次改动保持最小化

4\. \*\*验证行为不变\*\*：确认逻辑等价，不引入新功能或删除现有功能

通过 AI 来创建

在当前.claude 目录下创建一个命令(slash commands),用于代码简化，分析的维度如下：
...
这里特别要注意的是一定要用slash commands去去指定，不然的话会创建成技能

如果需要创建全局的命令，那就告诉他是在用户目录下

在用户的.claude 目录下创建一个命令(slash commands),用于代码简化，分析的维度如下：
