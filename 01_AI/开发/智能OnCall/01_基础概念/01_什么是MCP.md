...
![image\.png](../attachments/FgCibV486o7mj9xJS6kcJvVBnzC.png)
...
![image\.png](../attachments/Gl0rbBH2voS8iFxTCCWcbG7wnYe.png)
...
![image\.png](../attachments/JNwIbRnwIoqhbGxhfwWcS0ysncc.png)
...
![image\.png](../attachments/F8NJbggC1oWwORxzIsncKlecnyb.png)
...
![image\.png](../attachments/PRWbb4xDro5S30xFSMSc1ei9ndd.png)
...
1. Agent 里的 MCP Client 收到这条 Function Calling 指令，通过 **MCP 协议 **找到天气 MCP Server，把请求路由过去。这是 MCP 层，Agent 在找到并执行工具。

1. 天气 MCP Server 调用真实的天气 API，拿到结果，按 MCP 格式回传。

1. 大模型收到结果，整理成自然语言告诉用户。
...
![image\.png](../attachments/Xj4dbAorloM1HCxu5VWcd2Pmn4d.png)

1. **用户提问 **：在 Host（比如 Claude Desktop）里输入「帮我查一下 React 仓库最近的 commit」

1. **Host 分析任务 **：大模型判断需要调用 GitHub 工具，生成 Function Call 格式的调用指令

1. **Client 接收指令 **：Host 把调用指令交给内置的 MCP Client

1. **Client 路由到对应 Server **：Client 根据工具名，找到负责 GitHub 能力的 MCP Server，把请求发过去

1. **Server 执行 **：GitHub MCP Server 调用 GitHub API，拿到最近的 commit 列表

1. **结果回传 **：Server 把结果按 MCP 协议格式回传给 Client，Client 转交给 Host

1. **Host 生成回复 **：大模型拿到结果，整理成自然语言回复给用户
