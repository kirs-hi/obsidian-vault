...
![image\.png](../attachments/Z2p4bRu4OoIV4TxEeAycFK3cn8c.png)
...
![image\.png](../attachments/Y2LMb9VQhoBiMVxHW8FcIFkKnIL.png)
...
![image\.png](../attachments/I0L6bS7FJo8kaSxnBwKcEMOZnVh.png)
...
![image\.png](../attachments/LKSRbGfHToNtKvxsQpzcAEHjnQh.png)
...
1. 实现基于 Server\-Sent Events（SSE）的流式响应机制，配合前端实现 AI 对话实时输出体验，用户响应时间缩短至平均 60ms 内；

1. 构建融合文档向量检索与 Web Search 插件的 LangChain4j RAG 系统，提升 AI 回答的准确性与时效性，适配搜索增强问答场景；

1. 实现动态配置中心，支持通过前端界面无感刷新大模型 API Key、系统角色参数、向量数据库连接配置，避免频繁重启服务；

1. 设计并实现 Function Call 能力，通过可扩展 Tool 类体系封装本地方法，支持 AI 自动调用数据库检索、文件解析、第三方接口等操作；

1. 完善知识库上传模块，集成 MinIO 存储上传文档，自动完成文档解析与向量化入库流程，支持批量上传、内容过滤与多知识库管理。
...
- 完成知识库与问答接口部署，RAG 推理平均响应时间控制在 **800ms 内 **；自动评审组件已开源至 GitHub： [开源地址 🔗](https://my.feishu.cn/https%3A%2F%2Fgithub.com%2FLevionyx%2Fopenai-code-review%2Freleases%2Ftag%2FV1.0)
...
![image\.png](../attachments/JnAJbhFWdoyoctxGxmqcXJoAnqf.png)

## 模板21

![image\.png](../attachments/JuwubDQfjorGN1xYgdXcPHlIn6h.png)
...
![image\.png](../attachments/Nznvb0iVZo3agsxAKNGcDtBRnkf.png)
...
![image\.png](../attachments/UBkPbLVgZoEAgYxMvhrcps8ynkh.png)

技术上来说，大模型的 API 调用其实跟我们平时调用其他第三方服务差不多，主要是 HTTP 接口的对接。

[✅ 技术派集成 DeepSeek 到派聪明 AI 助手](https://my.feishu.cn/https%3A%2F%2Fwww.yuque.com%2Fitwanger%2Faz7yww%2Ftyb1f9pqraqoadq2)

不过在实际集成的时候需要考虑一些问题，比如响应时间比较长、成本控制、数据安全性等等。我们最近在调研一些开源的大模型，比如说 DeepSeek 的轻量级版本，看看能不能部署在自己的服务器上。

![image\.png](../attachments/HW7ob3UC9o0WD9xPsyJcFRaGn2f.png)
...
![image\.png](../attachments/MxJtbA6QEolYHFx26dDcQxVFnFg.png)
...
![image\.png](../attachments/CFW9bTrTyooWBZxxmQcckTqFncg.png)
...
![image\.png](../attachments/M9Yeb5ftKoYkoXxIL7ncj5zgnnh.png)
...
![image\.png](../attachments/Ujl6b8iFAo79roxiPSUcAi4Pnzf.png)
...
![image\.png](../attachments/CR01bZH1Xo44JOxORElc1KoNn2b.png)
...
![image\.png](../attachments/JLN1bhMb6oLI2Qxtpi7cdqK4nXE.png)

MCP 就像是给 AI 应用定义了一套"数据接口标准"，有点像我们后端开发中的 REST API 规范一样。通过这个标准，不同的数据源和工具都可以用统一的方式暴露给大模型，大模型也能用统一的方式来访问这些资源。

![image\.png](../attachments/PEKobm8AXoK57xxXKCNcFTXnnJc.png)
...
![image\.png](../attachments/NjfsbFVPKoQ6ljxUjqZciDoJn3e.png)
...
![image\.png](../attachments/FNWJb36G8oPzMwxO2JeceCPTnjb.png)
...
![image\.png](../attachments/JQRdbuUjzoxLqdxYZZgcIt88nUe.png)
...
> 星球原贴： [https://t\.zsxq\.com/D8EmY](https://my.feishu.cn/https%3A%2F%2Ft.zsxq.com%2FD8EmY)

![image\.png](../attachments/Oh0IbVy5borreaxOOJxcWRqQnLf.png)
...
![image\.png](../attachments/Vqk3b0mx3oWzkAxnqM4cu0iuneg.png)
...
![image\.png](../attachments/GXjebwkmcoI8hixgsyrcUzSjnxh.png)
...
计算公式为： RRF\_Score\(doc\) \= Σ \(1 / \(k \+ rank\_i\)\) rank\_i 是文档 doc 在第 i 个排名列表中的排名（比如在 BM25 结果中排第 3，在 KNN 结果中排第 10）。

![image\.png](../attachments/M3IxbB1F5oEAMoxiUvbclOQUnTs.png)
...
![image\.png](../attachments/XsjnbiMlCowGplxJnzBcnM5En0f.png)
...
> 星球原贴： [https://t\.zsxq\.com/P0GBc](https://my.feishu.cn/https%3A%2F%2Ft.zsxq.com%2FP0GBc)

![image\.png](../attachments/Nrggbur46os58ExL0rbcmFHmnkf.png)
...
![image\.png](../attachments/GDltbCaWDorm3CxtTafclizWnBc.png)
...
![image\.png](../attachments/W0swbPCVuoTPHexst1pcReBznKg.png)
...
![image\.png](../attachments/Up0pbjqP6oGKS1xEoTscKFwsnEf.png)
