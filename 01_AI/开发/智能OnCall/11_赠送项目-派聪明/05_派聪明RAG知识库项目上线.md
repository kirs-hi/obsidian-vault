# ✅派聪明 RAG 知识库项目上线了！AI 时代，你值得拥有

大家好，我是二哥呀。

历经 4 个月， 派聪明 RAG 知识库项目 终于完工（包括源码和教程，全部交付给大家）！

不容易啊。

这篇内容我先带大家了解一下什么是派聪明，我为什么要做派聪明这个企业级的 RAG 知识库？派聪明这个 AI 项目能让大家学到什么？以及如何解锁派聪明的源码仓库和教程？

![image\.png](../attachments/PG6ubZ7yfot38AxMR0EcBh5fnlf.png)

## 一、派聪明的起源

了解技术派的球友应该知道，派聪明最初是 [技术派实战项目 ](https://my.feishu.cn/https%3A%2F%2Fjavabetter.cn%2Fzhishixingqiu%2Fpaicoding.html)中一部分，主打的服务是和 AI 大模型进行聊天对话（没有检索增强生成）。

技术派这个项目也是非常经典，帮助 24、25 届，以及 26 届同学斩获了很多超出预期的 offer，我们这里就不多讲了，只贴一个喜报，一位球友靠技术派和面渣逆袭，投了两周拿下小红书的日常实习。

![image\.png](../attachments/Vqhjb7PSoook16xbr2rc2Ivonvj.png)

技术派这个项目如果你能认真学完，会收获非常非常多，基本上一个企业级的 Web 开发项目用到的技术栈，技术派都覆盖到了。

![image\.png](../attachments/AwMQbxgO1owlbExmcgIczjMQn0f.png)

那为了延续正宗的皇家血脉，我把 **派聪明 **这三个字延续到了新的 RAG 项目，也就是大家目前看到的这个派聪明 AI 知识库项目（界面还是非常清爽的 😄）。

![image\.png](../attachments/QavobS8D6oslOZxgVtscX8bondh.png)
...
![image\.png](../attachments/XOiCbFqnxoMQHTx8btYc1EePnHU.png)
...
- AI 集成 : DeepSeek API/本地 Ollama\+豆包 Embeddin g
...
![image\.png](../attachments/LKw2bNfqtoKTnrxbndKcAvhUnMe.png)
...
![image\.png](../attachments/SWsobWTGTofUJqxMK5gcVxcfnJd.png)

具体来说，系统会使用 Apache Tika 从文档中提取出纯文本信息，然后将长文档智能切分成多个语义完整的文本块。

![image\.png](../attachments/MLNkbSnINoKz3JxyjWncsCFjnlc.png)
...
![image\.png](../attachments/H8aNbT9bvoDAypx8hqEc8Patn3e.png)
...
![image\.png](../attachments/DRcfbkr67oencyxPNkJcI2I5nKb.png)

![image\.png](../attachments/L3jLbfxXIo6q1GxxaTkcXBWNn4g.png)

![image\.png](../attachments/COqzbLfVmofIYJxFWikcAamynvv.png)
...
![image\.png](../attachments/SzFQbPaPdoFW6ex55sLcJTH1nBd.png)
...
![image\.png](../attachments/RtE1bHyX9oFz9DxBk01cYihcnFc.png)

![image\.png](../attachments/GEubbDeBaoh3eJxBLMocRhISn2d.png)
...
![image\.png](../attachments/RKrqbQCuWorLeSxknsfc4EuTnnc.png)
...
![image\.png](../attachments/Tx0wbCLOZoMdbIx9q5Tcx6PRnNg.png)
...
![image\.png](../attachments/PXRXbsnqsouUvbxUzuocVmfKnhc.png)
...
![image\.png](../attachments/YsDMb2AiToc3FOx65xAc2WqsnKe.png)
...
![image\.png](../attachments/VNElbczJfoegz1xKAQPcfAgFnAf.png)

那基于向量知识库的语义检索，虽然解决了传统关键词匹配的局限性，但显然关键词搜索这种场景还是需要的，所以派聪明兼具了关键词检索和语义检索两种能力。

![image\.png](../attachments/BBiob19SQoU2PrxXnbscCuHYnsO.png)
...
![image\.png](../attachments/XCKObuUmdouIqMx1mypcHY6WnBf.png)

### 02、Kafka\+Redis\+MinIO

派聪明通过 Kafka 实现异步任务调度，使用 Redis 缓存优化对话上下文管理，基于 MinIO 实现大文件分片上传与断点续传。

Redis 的八股就不用多说了，有 [面渣逆袭 Redis 篇 ](https://my.feishu.cn/https%3A%2F%2Fjavabetter.cn%2Fsidebar%2Fsanfene%2Fredis.html)，技术派和 PmHub 中都没有用 Kafka，用的的 RabbitMQ 和 RocketMQ，这次把消息队列中的 Kafka 直接补齐，从此以后再也不用担心 MQ 没有落地经验了（齐活）。

![image\.png](../attachments/QscAbG9kyolBgcx2XTcc0oTknjf.png)

MinIO 的话，在处理文件的时候也经常用到， [编程喵当时有讲到 ](https://my.feishu.cn/https%3A%2F%2Fwww.yuque.com%2Fitwanger%2Fvn4p17%2Fta5vr1)，但技术派和 PmHub 中都没有应用，这次派聪明我们也一并补齐了。

![image\.png](../attachments/NI5VbPReroVaENxFuSNcRqlqnIc.png)

### 03、ElasticSearch

集成 Elasticsearch，实现「关键词\+语义」的双引擎搜索。目前已经通过 ES 的 bool should 查询实现了关键词\+语义的搜索方式。

![image\.png](../attachments/T4bHbY5Y0orajpxv8wFcQZsCngd.png)

### 04、Vue 3\+TypeScript\+Vite

派聪明全面采用了 Vue 3 的 Composition API，技术派当时用的是 React，等于说前端的三驾马车就只剩下了 Angular。

![image\.png](../attachments/Te0SbE78Joa64lxEWGxcsJrQnUh.png)
...
![image\.png](../attachments/E0RVb9691ocRzVx23CEcotOBnMf.png)
...
![image\.png](../attachments/PVfKbq8QyoGx8jx9roVc9G9EnGe.png)
...
![image\.png](../attachments/DpqgbIlIwoDsB6xLOsvcsTGanHb.png)
...
![image\.png](../attachments/VOrkb60P2o2w39x1lr3cV81nnZg.png)
...
![image\.png](../attachments/VRlOb19ZpoJzbwxfl1LcIDpCnFb.png)

## 八、解锁派聪明源码\+教程

那这次为了避免盗版，这次的代码仓库采用的是邀请制，加入星球后，在星球第一个置顶帖【球友必看】中获取邀请链接，审核通过后即可查看。

![image\.png](../attachments/OMqRbamD5oQgtAxPdaWcnyFwnfh.png)

![image\.png](../attachments/P41VbUrCDo1noexPWA2cQ0u8nle.png)

![image\.png](../attachments/BYcabxJdKoeqFIxzR47c5yGanIb.png)

这里再次重申下，加入 [「二哥的编程星球」 ](https://my.feishu.cn/https%3A%2F%2Fjavabetter.cn%2Fzhishixingqiu%2F)后，你还可以享受以下专属内容服务：

- 1、 **付费文档: **派聪明 RAG、 [微服务 PmHub ](https://my.feishu.cn/https%3A%2F%2Flaigeoffer.cn%2Fpmhub%2Flearn%2F)、 [前后端分离技术派 ](https://my.feishu.cn/https%3A%2F%2Fjavabetter.cn%2Fzhishixingqiu%2Fpaicoding.html)、轮子 MYDB、入门编程喵、AI\+MCP 的校招派等项目配套的 60 万\+ 字教程查看权限

- 2、 **简历修改 **: 提供价值超 600 元的 [简历修改服务 ](https://my.feishu.cn/https%3A%2F%2Fjavabetter.cn%2Fzhishixingqiu%2Fjianli.html)，附赠星球 5000\+优质简历模板可供参考

- 3、 **专属问答 **: 向二哥和星球嘉宾发起 1v1 提问，内容不限于 offer 选择、学习路线、职业规划等

- 4、 **面试指南 **: 获取针对校招、社招的 40 万\+字面试求职攻略《 [Java 面试指南 ](https://my.feishu.cn/https%3A%2F%2Fjavabetter.cn%2Fzhishixingqiu%2Fmianshi.html)》，以及二哥的 LeetCode 刷题笔记、一灰的职场进阶之路、华为 OD 题库
